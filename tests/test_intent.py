...
# --- 커밋 축: 고정 문구가 아니라 (시점어 + 변화어) 로 잡는다 --------------------
# 처음에는 "최근에 바뀐" 같은 **고정 문구**를 나열했는데, 사이에 수식어가 끼면 샜다
# (실측: "최근에 가장 크게 바뀐 부분이 뭐야?" → unknown). 문구를 계속 늘리는 것은
# 본 질문에만 맞추는 과적합이라, 동시 출현 규칙으로 일반화했다.

@pytest.mark.parametrize("q", [
    "최근에 가장 크게 바뀐 부분이 뭐야?",     # 사이에 수식어가 끼어도
    "언제 도입됐어?",                          # 변화어가 '도입' 이어도
    "이 값은 예전과 지금이 어떻게 달라졌고 언제 바뀌었어?",
    "마지막으로 수정된 게 언제야?",
])
def test_시점어와_변화어가_함께면_커밋축(q):
    assert classify(q).axis == "commit"


@pytest.mark.parametrize("q", [
    "설정값을 어떻게 변경해?",     # 변화어만 — 사용법 질문이지 이력 질문이 아니다
    "청크를 추가하려면?",
])
def test_변화어만으로는_커밋축으로_보지_않는다(q):
    # 변화어 하나로 잡으면 "어떻게 바꿔?" 류 사용법 질문이 전부 커밋으로 끌려간다.
    assert classify(q).axis != "commit"


def test_명시_어휘는_단독으로_커밋축():
    assert classify("변경 이력을 알려줘").axis == "commit"
    assert classify("최근 커밋에서 뭘 했어?").axis == "commit"
