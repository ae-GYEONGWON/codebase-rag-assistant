...
def cmd_sample(n: int, seed: int) -> None:
    ...
    L.append("")
    L.append("각 항목의 `verdict:` 줄에 아래 하나를 적으세요. 다 채운 뒤 `python -m eval.verify score`.")
    L.append("")
    L.append("| 판정 | 뜻 |")
    L.append("|---|---|")
    L.append("| `ok` | 조각이 질문에 답한다. 라벨 유효 |")
    L.append("| `elsewhere` | 조각도 답하지만 **다른 파일에도 답이 있다** — `also:` 줄에 파일 적기 |")
    L.append("| `wrong` | 조각이 질문에 답하지 못한다 → 폐기 |")
    L.append("| `unclear` | 질문 자체가 모호하다 → 폐기 |")
    L.append("")
    L.append("> ⚠️ 이 워크시트에는 **검색기가 무엇을 찾았는지 넣지 않았다.** 그걸 보고 판정하면")
    L.append("> 검수가 검색기를 정당화하는 절차가 되어 버린다. 질문과 조각만 보고 판정할 것.")
    L.append("")
    L.append("---")
    L.append("")
