...
메모리**라고 적었다. 그런데 그 주장은 **3,619 청크까지만 재고 10만 개일 때를 계산으로 밀어
추정한 값**이었다.

잰 범위 밖의 추정은 근거가 못 된다 — 그 밖에서는 성질이 달라질 수 있기 때문이다(행렬이 CPU
캐시에 안 들어가거나, 메모리가 부족해 디스크로 밀리면 어느 지점부터 갑자기 느려진다).
그래서 N 을 실제로 키워 가며 잰다.
...
def render(res: dict) -> str:
    ...
    out = ["# ANN 전환 임계 실증 — 브루트포스는 언제까지 버티는가", "",
           ...
           "잰 범위 밖에서는 성질이 달라질 수 있어 실제로 키워 가며 다시 쟀다.", "",
