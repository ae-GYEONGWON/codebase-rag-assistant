...
- **비개발자 UX**: 개발자 모드 토글(코드 인용 on/off) + 피드백 👍/👎 → **완료**
- **개발자 모드 실동작화**: 코드 심볼 검색 + dev_mode 프롬프트 분기 → **완료**
- **Tier 3 (도장)**: 엔지니어링 노트 + Docker + pytest(21) + README 전면개편 → **완료**

## 상태: MVP 로드맵 사실상 완료. 남은 건 선택적 고도화.

## 다음 작업 (선택적 — 사용자와 재합의 필요)
1. (후보) 최신성 가중치 — commit_date 로 시간질문("언제/최근") 개선. 단 특정날짜 질문
   ("2026-06-26 사고")을 망칠 리스크 → 측정 설계 후 결정(리랭커 교훈: 측정 없이 넣지 말 것)
2. (후보) 코드질문 미스 2건("SL 코드계산", "웹 API 라우터") — 의도 라우팅
3. (후보) 증분 재인덱싱(변경분만) / CI 로 eval 회귀 게이트
4. (후보) GitHub 공개 push (현재 로컬 커밋만)
5. **면접 대비 문서** — 사용자가 직접 써보며 RAG/벡터DB 개념 익힌 후 작성(docs/engineering-notes.md 가 재료)

## Docker/pytest 메모
- Docker: 이 환경에 미설치라 **빌드 자체는 미검증**. Dockerfile/compose 문법은 확인됨.
  컨테이너=쿼리서빙 전용(chroma_db 마운트), 인덱싱은 호스트. INDEX_CODE/GIT=false.
- pytest: 21개 통과(순수로직 + 검색통합). 통합은 chroma 없으면 skip.
...
지식원 ─┬ .md 문서(memory/**) → 헤더 인지 분할
   ...
   질문 ─┬→ 벡터 검색(코사인)  ─┐
         └→ BM25(어휘)        ─┴→ RRF 융합 → MMR(λ=1.0) → 범위밖 게이트(cos≥0.35)
...
- **mmr_lambda=1.0**(다양성 off): λ 스윕 결과. 문서만(956청크) 땐 0.8 최적이었으나 코드+git 확장
  (3619청크) 후 재측정 0.9→86% / 0.95→89% / 1.0→93% → 1.0 채택. 코퍼스 커지자 유사청크↑로
  다양성이 정답을 밀어냄. (하이퍼파라미터는 데이터 종속 — 코퍼스 바뀌면 재측정. 엔지니어링노트 #13)
